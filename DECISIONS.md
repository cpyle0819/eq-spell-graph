...
The same "don't trust classic-EQ memory" warning applies to spell tuning, not just zones — e.g. Ward Undead (Paladin L1) shows "35 and 41 damage" in our data, which doesn't match live-EQ memory of the spell. Investigated by fetching eqlwiki.com's live page directly: the wikitext's own two fields *disagree with each other* right now — the free-text `description` field says "inflicts 12 damage" while the structured `{{SpellSlotRow}}` effect table says "Decrease Hitpoints by 35 (L5) to 41 (L11)." Our scraper (`scripts/scrape-spell-details.ts`) pulls the `description` field verbatim, and our stored value ("between 35 and 41 damage... 1.37 DPM") matches neither of the wiki's current fields — meaning the wiki's prose was edited (likely a rebalance) sometime after our last scrape, and the structured slot table either wasn't updated in sync or is the one still correct. Not a scraper bug: the scraper faithfully copies whatever the `description` field said at scrape time. Lesson: this wiki's own fields can drift out of sync with each other mid-edit, not just lag behind the game — if a number looks wrong, check both the live wiki page's prose *and* its structured slot table before assuming either our data or your memory is at fault, and consider re-running the scraper if it's been a while.
